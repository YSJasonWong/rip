...
if (isset($_POST['username'])) {
    ...
    if (empty($error)) {
        ...
        header ("location: SuccessRegZh.php");
    } else {
        echo $error;
        echo "<script>alert('".$error."');</script>";
        header("Location: registerZh.php" ,true ,303);
    }
}
        ...
        <div class="row vh-100 align-items-center justify-content-center py-5">
            <div class="col-md-6 py-5">
                <form name="form" class="font-1 pb-5" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                    <legend class="text-black fs-2 fw-bold text-start">註冊新用戶</legend>
                    <label>用戶名稱: </label><input class="m-2" type="text" id="username" name="username" maxlength="20" size="15" pattern="^([a-z]+[A_Z]+[0-9]+){8-20}$" required><p>需由英文字母及數字組成</p>
                    <label>匿稱: </label><input class="m-2" type="text" id="nickname" name="nickname" maxlength="20" size="15" required><br>
                    <label>電郵: </label><input class="m-2" type="email" id="email" name="email" maxlength="50" size="30" pattern="^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$" required><br>
                    <label>密碼: </label><input class="m-2" type="password" id="pwd" name="pwd" maxlength="20" size="20" pattern="^[(a-z)+(A-Z)+(0-9)+(\.\,\^\&\*\@\#\$\%)+]{8-20}$" onchange="checkPasswordinput(this.value);checkPasswordMatch();" required><br>
                    <ul>
                    您的密碼需:
                        <li id="length">由8-20字母組成</li>
                        <li id="upChar">最少包含一個大寫英文字母</li>
                        <li id="lowChar">最少包含一個小寫英文字母</li>
                        <li id="num">最少包含一個數字</li>
                        <li id="specialChar">最少包含一個特殊字元: .,^&*@#$%</li>
                    </ul>
                    <script>
                    ...
                    <label>再次輸入密碼: </label><input class="m-2" type="password" id="pwd2" name="pwd2" maxlength="20" size="20" onchange="checkPasswordMatch()" required><br>
                    <ul><li id="matched">密碼相乎</li></ul>
                    <input class="btn bg-black text-1 my-3" type="submit" name="submit" value="註冊">
                </form>
            </div>
        </div>
    </div>
